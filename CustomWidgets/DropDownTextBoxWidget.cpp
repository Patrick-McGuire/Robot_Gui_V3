...
void DropDownTextBoxWidget::updateInFocus() {
    if (widgetData->keyUpdated(configInfo->source)) {
        if (widgetData->getKeyType(configInfo->source) == WidgetData::json_t && widgetData->getJSON(configInfo->source)->getType() == InternalJson::map_t) {
            auto pages = widgetData->getJSON(configInfo->source);

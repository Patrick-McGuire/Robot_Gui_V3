...
RobotGui::VideoWidget::VideoWidget(QWidget *parent, const RobotGui::WidgetBaseConfig::SharedPtr &configInfo, RobotGui::WidgetData *widgetData, RobotGui::Theme *_theme) :
        BaseWidget(parent, configInfo, widgetData, _theme) {
    configInfo->require(WidgetConstants::SOURCE);
...
void RobotGui::VideoWidget::customUpdate() {
    if (configInfo->source.is_initialized()) {
        if (widgetData->imgExits(configInfo->source.get())) {
            ...
            videoWidget.setPixmap(
                    QPixmap::fromImage(QImage((unsigned char *) rgb_image.data, rgb_image.cols, rgb_image.rows, QImage::Format_RGB888)).scaled(configInfo->width.get(), configInfo->height.get(),
                                                                                                                                               Qt::KeepAspectRatio));
        }
    }
}
